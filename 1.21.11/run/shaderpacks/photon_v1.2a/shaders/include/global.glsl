...
// Cataclysm Weather Uniforms
uniform int stormCount;
uniform vec4 stormPositions[16]; // xyz: position, w: radius/level
uniform float stormStages[16]; // formationProgress
uniform float tornadoWidths[16]; // Level of the tornado, used for scaling its width
uniform float cata_world_age; // Current world age from the mod
